...
The notebooks in `nbs/*.ipynb` are the canonical source. Run
`nbdev_prepare` (or just `nbdev_export`) after editing a notebook to
regenerate the corresponding `.py` file in `gcpeasy/`. Test cells in the
notebooks are marked `#| hide` so they execute under `nbdev_test` but are
neither exported to the package nor rendered in the docs.
---
