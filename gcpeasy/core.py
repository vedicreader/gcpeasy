...
class GCPAuth:
    ...
    def __init__(self, project=None, region=None, service_account_file=None,
        ...
        self.impersonate_sa = impersonate_sa
        if not self.project:
...
def list_resources(auth) -> list:
    """List all labeled resources in the project."""
    return find_resources(auth, {})


# Deprecated aliases — kept for backward compatibility
def label_resources(auth, labels: dict) -> list:
    "Deprecated: use `find_resources` instead."
    return find_resources(auth, labels)


def list_labeled_resources(auth) -> list:
    "Deprecated: use `list_resources` instead."
...
class GenAIStack:
    ...
    def provision(
        ...
        from gcpeasy.network import create_service_account, bind_iam_role, create_secret, enable_audit_logs
        ...
        if co.get('audit'):
            enable_audit_logs(self.auth)
            self._resources['audit_logs'] = 'enabled'
        ...
        if secret_manager:
            sec = create_secret(self.auth, f'{name}/api-key', 'placeholder', labels=labels)
